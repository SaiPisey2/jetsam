...
// trailing newline. It lives here rather than in edit.go because nothing in
// this package needs it outside tests -- callers elsewhere already have the
// line they want to check without re-reading the file.
...
// (typically a `# comment`) after it. It lives here rather than in edit.go
// because it is test scaffolding for building a realistic ReplaceWith
// input -- the package itself has no notion of a `for:` field.
...
// formatDuration renders d the way a `for:` value reads in a rule file --
// compact units, no zero components -- rather than time.Duration.String's
// noisier form (3m0s, 1h2m0s). Test scaffolding only; see replaceForValue.
...
func TestApplyDeletePreservesEverythingOutsideTheSpan(t *testing.T) {
	...
	for _, must := range []string{
		"# Fixture rule file for internal/forge's diff-generation tests",
