...
func (g *GitHubProvider) EnsureBranch(ctx context.Context, owner, repo, branch, base string) error {
	...
	// permission to create it. See apiError for what comparing message
	// text instead used to let through.
...
func (g *GitHubProvider) CommitFiles(ctx context.Context, owner, repo, base, branch string, changes []FileChange) error {
	for _, c := range changes {
		...
		fp, err := filePath(c.Path)
		if err != nil {
			return err
		}
		if _, err := g.do(ctx, http.MethodPut, rp+"/contents/"+fp, body, nil); err != nil {
